...
export default class SimpleReactValidator {
  ...
  constructor(options = {}) {
    ...
    this.rules = {
      accepted: { message: 'Your :attribute must be accepted.', rule: val => val === true, required: true },
      after: {
        message: 'Your :attribute must be after :date.',
      ...
      after_or_equal: {
        message: 'Your :attribute must be after or on :date.',
        rule: (val, params) =>
          this.helpers.momentInstalled() && moment.isMoment(val) && val.isSameOrAfter(params[0], 'day'),
        messageReplace: (message, params) => message.replace(':date', params[0].format('MM/DD/YYYY')),
      },
      alpha: {
        message: 'Your :attribute may only contain letters.',
        rule: val => this.helpers.testRegex(val, /^[A-Z]*$/i),
      },
      alpha_space: {
        message: 'Your :attribute may only contain letters and spaces.',
        rule: val => this.helpers.testRegex(val, /^[A-Z\s]*$/i),
      },
      alpha_num: {
        message: 'Your :attribute may only contain letters and numbers.',
        rule: val => this.helpers.testRegex(val, /^[A-Z0-9]*$/i),
      },
      alpha_num_space: {
        message: 'Your :attribute may only contain letters, numbers, and spaces.',
        rule: val => this.helpers.testRegex(val, /^[A-Z0-9\s]*$/i),
      },
      alpha_num_dash: {
        message: 'Your :attribute may only contain letters, numbers, and dashes.',
        rule: val => this.helpers.testRegex(val, /^[A-Z0-9-]*$/i),
      },
      num_dash: {
        message: 'Your :attribute may only contain numbers, and dashes.',
        rule: val => this.helpers.testRegex(val, /^[0-9-]*$/i),
      },
      alpha_num_dash_space: {
        message: 'Your :attribute may only contain letters, numbers, dashes, and spaces.',
        rule: val => this.helpers.testRegex(val, /^[A-Z0-9-\s]*$/i),
      },
      array: { message: 'Your :attribute must be an array.', rule: val => Array.isArray(val) },
      before: {
        message: 'Your :attribute must be before :date.',
        rule: (val, params) => this.helpers.momentInstalled() && moment.isMoment(val) && val.isBefore(params[0], 'day'),
        messageReplace: (message, params) => message.replace(':date', params[0].format('MM/DD/YYYY')),
      },
      before_or_equal: {
        message: 'Your :attribute must be before or on :date.',
        rule: (val, params) =>
          this.helpers.momentInstalled() && moment.isMoment(val) && val.isSameOrBefore(params[0], 'day'),
        messageReplace: (message, params) => message.replace(':date', params[0].format('MM/DD/YYYY')),
      },
      between: {
        message: 'Your :attribute must be between :min and :max:type.',
        rule: (val, params) =>
      ...
      boolean: { message: 'Your :attribute must be a boolean.', rule: val => val === false || val === true },
      card_exp: {
        message: 'Your :attribute must be a valid expiration date.',
        rule: val => this.helpers.testRegex(val, /^(([0]?[1-9]{1})|([1]{1}[0-2]{1}))\s?\/\s?(\d{2}|\d{4})$/),
      },
      card_num: {
        message: 'Your :attribute must be a valid credit card number.',
        rule: val => this.helpers.testRegex(val, /^\d{4}\s?\d{4,6}\s?\d{4,5}\s?\d{0,8}$/),
      },
      currency: {
        message: 'Your :attribute must be a valid currency.',
        rule: val => this.helpers.testRegex(val, /^\$?(\d{1,3})(\,?\d{3})*\.?\d{0,2}$/),
      },
      date: {
        message: 'Your :attribute must be a date.',
        rule: val => this.helpers.momentInstalled() && moment.isMoment(val),
      },
      date_equals: {
        message: 'Your :attribute must be on :date.',
      ...
      integer: { message: 'Your :attribute must be an integer.', rule: val => this.helpers.testRegex(val, /^\d*$/) },
      max: {
        message: 'Your :attribute may not be greater than :max:type.',
      ...
      min: {
        message: 'Your :attribute must be at least :min:type.',
        rule: (val, params) => this.helpers.size(val, params[1]) >= parseFloat(params[0]),
        messageReplace: (message, params) =>
      ...
      not_regex: {
        message: 'Your :attribute must not match the required pattern.',
        rule: (val, params) =>
      ...
      phone: {
        message: 'Your :attribute must be a valid number.',
        rule: (val, params, validator) => {
          let pNo = val.replace(/[-|+]/g, '');
          return (
            validator.helpers.testRegex(
              val,
              /^(\+?\d{0,4})?\s?-?\s?(\(?\d{3}\)?)\s?-?\s?(\(?\d{3}\)?)\s?-?\s?(\(?\d{4}\)?)?$/,
            ) &&
            validator.helpers.testRegex(pNo, /^(?!(\d)\1{9})\d{10}$/) &&
            pNo.length == 10
          );
        },
      ...
      password: {
        ...
        rule: (val, params, validator) => {
          return (
            validator.helpers.testRegex(val, /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[-_<,.>#$^+=!*()@%&]).{8,20}$/) &&
      ...
      regex: {
        message: 'Your :attribute must match the required pattern.',
        rule: (val, params) =>
          this.helpers.testRegex(
            val,
            typeof params[0] === 'string' || params[0] instanceof String ? new RegExp(params[0]) : params[0],
      ...
      required: {
        message: 'Your :attribute field is required.',
        rule: val => !this.helpers.isBlank(val),
        required: true,
      },
      size: {
        message: 'Your :attribute must be :size:type.',
        rule: (val, params) => this.helpers.size(val, params[1]) == parseFloat(params[0]),
        messageReplace: (message, params) =>
          message.replace(':size', params[0]).replace(':type', this.helpers.sizeText(params[1])),
      },
      string: { message: 'Your :attribute must be a string.', rule: val => typeof val === typeof 'string' },
      typeof: {
        message: 'Your :attribute is not the correct type of :type.',
        rule: (val, params) => typeof val === typeof params[0],
        messageReplace: (message, params) => message.replace(':type', typeof params[0]),
      },
      url: {
        message: 'Your :attribute must be a url.',
        rule: val =>
          this.helpers.testRegex(
            val,
    ...
    if (options.element === false) {
      ...
      this.element = (message, className) =>
        React.createElement('div', { className: className || this.className || 'srv-validation-message' }, message);
    }
  }
